...
echo ""
echo "Domain and TLS"
CURRENT_DOMAIN="$(yaml_get "$LOCAL_CONFIG_FILE" domain)"
[[ -n "$CURRENT_DOMAIN" ]] || CURRENT_DOMAIN="$(yaml_get "$CONFIG_FILE" domain)"
...
CURRENT_EMAIL="$(yaml_get "$LOCAL_CONFIG_FILE" acmeEmail)"
[[ -n "$CURRENT_EMAIL" ]] || CURRENT_EMAIL="$(yaml_get "$CONFIG_FILE" acmeEmail)"
...
render_tfvars | sed 's/^/    /'
# The environment's answers file: same schema as the overlay config.yaml, but
# only the keys the interview answered. cfg_get lays it over the committed
# sample at read time, so no committed file is ever edited.
render_local_config() {
    echo "# Generated by setup-env.sh for ${ENV}. Not committed (gitignored)."
    echo "# Values here override k8s/overlays/${ENV}/config.yaml (the committed sample)."
    echo "domain: ${DOMAIN_IN}"
    echo "acmeEmail: ${ACME_EMAIL_IN}"
    echo "gitops:"
    echo "  repoUrl: https://git.${DOMAIN_IN}/devhub/devhub.git"
    if [[ "$TIER" == "workload" ]]; then
        # Workload clusters point at the platform cluster; strip the leading
        # app subdomain to guess it, right for the documented apps.<domain> layout.
        local platform_domain="$DOMAIN_IN"
        [[ "$DOMAIN_IN" == *.*.* ]] && platform_domain="${DOMAIN_IN#*.}"
        echo "platformVaultUrl: https://vault.${platform_domain}"
        echo "platformLokiUrl: https://loki.${platform_domain}"
    fi
}

echo ""
log_step "config.yaml → ${LOCAL_CONFIG_FILE#"${REPO_ROOT}/"}  (overrides the committed overlay)"
render_local_config | sed 's/^/    /'
echo ""
...
render_local_config > "$LOCAL_CONFIG_FILE"
chmod 600 "$LOCAL_CONFIG_FILE"
log_info "Wrote ${LOCAL_CONFIG_FILE#"${REPO_ROOT}/"} (domain, acmeEmail, gitops.repoUrl)"
log_info "The committed ${CONFIG_FILE#"${REPO_ROOT}/"} is untouched — it stays a sample/fallback"
